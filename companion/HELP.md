...
| Category           | Buttons                                                                                                                                                                                                                                                   |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Channels**       | One button per layout of every channel; red while that layout is active. Does not include a preview image (see _Channel: layout preview (active layout only)_ under Feedbacks if you want one on a specific button).                                      |
...
| **Outputs**        | One button per output and source (multi-viewer, device information, console, each channel, each video-capable input); blue (optimistic) while it matches the source last set from Companion.                                                              |
...
- **No live preview for a layout you are not on.** The API only exposes a live image of a channel's current output, not a stored thumbnail per layout, so _Channel: layout preview (active layout only)_ only ever shows an image for the active layout's button — there is no way to preview what a layout will look like before switching to it. Because of this, the _Channels_ presets do not attach it by default (with several layouts per channel, most of the buttons would just show a blank tile); add it to one button yourself if you want it. The _Previews_ category's per-channel button already shows a live image of whatever is currently on air, independent of which layout that is.
- **No preview or output routing for audio-only inputs.** An audio-only input (e.g. "HDMI-A Audio") has no picture, so it gets no preview button/feedback and is not offered as an output source. There is currently no VU meter feedback either, so audio-only inputs have no visual representation at all in Companion.
