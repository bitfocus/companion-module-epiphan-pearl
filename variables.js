module.exports = {
  updateVariables(self) {
    ...
    for (const cid of Object.keys(self.state.channels)) {
      ...
      const activeLayout = Object.values(channel.layouts || {}).find(
        (l) => l.active,
      );
      variables.push({
      ...
      values[`channel_${cid}_active_layout`] = activeLayout
        ? activeLayout.name
        : "";

      const videoEncoder = (channel.encoders || []).find(
        (e) => e.type === "video",
      );
      if (videoEncoder) {
        ...
        values[`channel_${cid}_resolution`] =
          encStatus.resolution ?? videoEncoder.resolution ?? "";
        values[`channel_${cid}_fps`] =
          encStatus.framerate ?? videoEncoder.framerate ?? "";
        values[`channel_${cid}_bitrate`] =
          encStatus.bitrate ?? videoEncoder.bitrate ?? "";
      }
      ...
      for (const pid of Object.keys(channel.publishers || {})) {
        ...
        if (pub.status?.statistics?.current?.send_rate !== undefined) {
          ...
          values[`stream_${cid}_${pid}_bitrate`] =
            pub.status.statistics.current.send_rate;
        }
      }
    }
