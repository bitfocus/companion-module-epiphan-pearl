...
if (typeof global.fetch !== "function") {
  throw new Error(
    "Global fetch API is not available. Please use Node 18 or newer",
  );
}
...
class EpiphanPearl extends InstanceBase {
  ...
  async configUpdated(config) {
    if (
      !config.host ||
      config.host.match(new RegExp(Regex.IP.slice(1, -1))) === null
    ) {
    ...
    if (
      !config.host_port ||
      parseInt(config.host_port) < 1 ||
      parseInt(config.host_port) > 65536
    ) {
      this.log(
        "error",
        "invalid portnumber given in configuration: " + config.host_port,
      );
      return;
    }
  ...
  async updateActiveChannelLayout(channelId) {
    ...
    const layouts = await this.sendRequest(
      "get",
      "/api/channels/" + channelId + "/layouts",
      {},
    );
  ...
  initInterval() {
    this.timer = setInterval(
      this.dataPoller.bind(this),
      Math.ceil(this.config.pollfreq * 1000) || 10000,
    );
  }
  ...
  async dataPoller() {
    ...
    try {
      const requests = [
        this.sendRequest(
          "get",
          "/api/channels?publishers=yes&encoders=yes",
          {},
        ),
    ...
    recorders_status.forEach((recorder) => {
      if (state.recorders[recorder.id] === undefined)
        state.recorders[recorder.id] = {}; // just for the event a recorder has been created between call to recorders and recorders/status
    ...
    await Promise.allSettled([
      ...
      ...channels.map(async (channel) => {
        const publishers = await this.sendRequest(
          "get",
          "/api/channels/" + channel.id + "/publishers/type",
          {},
        );
        publishers.forEach((publisher) => {
          ...
          state.channels[channel.id].publishers[publisher.id].type =
            publisher.type;
          state.channels[channel.id].publishers[publisher.id].name =
            publisher.name;
        });
      }),
      ...channels.map(async (channel) => {
        ...
        publishersstatus.forEach((publisher) => {
          if (state.channels[channel.id].publishers[publisher.id] === undefined)
            state.channels[channel.id].publishers[publisher.id] = {};
          state.channels[channel.id].publishers[publisher.id].status =
            publisher.status;
        });
      }),
    ]);
    ...
    if (
      ...
      JSON.stringify(Object.keys(this.state.channels))
    ) {
      updateNeeded = true;
    } else if (
      JSON.stringify(recorderIds) !==
      JSON.stringify(Object.keys(this.state.recorders))
    ) {
      updateNeeded = true;
    } else if (
      channelIds.reduce(
        (acc, curr) => `${acc},${state.channels[curr].name}`,
        "",
      ) !==
      channelIds.reduce(
        (acc, curr) => `${acc},${this.state.channels[curr].name}`,
        "",
      )
    ) {
      updateNeeded = true;
      ...
      recorderIds.reduce(
        (acc, curr) => `${acc},${state.recorders[curr].name}`,
        "",
      ) !==
      recorderIds.reduce(
        (acc, curr) => `${acc},${this.state.recorders[curr].name}`,
        "",
      )
    ) {
      updateNeeded = true;
    ...
    if (updateNeeded) {
      ...
      feedbacksToCheck = [
        ...
        "streamingState",
        "recorderRecording",
      ]; // recheck everything after reconfiguration, could be more fine grained but not worth for such a small amount of feedbacks
    } else {
      if (
        channelIds.reduce(
          (acc, curr) =>
      ...
      if (
        recorderIds.reduce(
          (acc, curr) =>
            `${acc},${JSON.stringify(state.recorders[curr].status.state)}`,
          "",
        ) !==
        recorderIds.reduce(
          (acc, curr) =>
            `${acc},${JSON.stringify(this.state.recorders[curr].status.state)}`,
          "",
        )
      ) {
    ...
    if (updateNeeded) {
      ...
      this.log(
        "info",
        "Pearl configuration has changed, Choices and Presets updated.",
      );
    }
  }
  ...
  firstId(arr) {
    if (
      Array.isArray(arr) &&
      arr.length > 0 &&
      (typeof arr[0].id === "string" || typeof arr[0].id === "number")
    ) {
      return arr[0].id;
    } else {
      return "";
  ...
  choicesChannelPublishers() {
    ...
    for (const channel of Object.keys(this.state.channels)) {
      if (Object.keys(this.state.channels[channel].publishers).length > 0) {
        ...
        for (const publisher of Object.keys(
          this.state.channels[channel].publishers,
        )) {
          choices.push({
  ...
  async updateRecorderStatus() {
    ...
    this.log(
      "debug",
      "Updating RECORDER_STATES and then call checkFeedbacks(recorderRecording)",
    );
  ...
  async fetchMetadata(channelId) {
    ...
    try {
      ...
      if (this.config.verbose) {
        this.log(
          "debug",
          `Parsed Metadata ${JSON.stringify(this.metadata[channelId])}`,
        );
      }
      variables.updateVariables(this);
    } catch (e) {
