...
class EpiphanPearl extends instanceSkel {
		...
		switch (action.action) {
			case 'channelChangeLayout': {
				...
				if (!this._getChannelById(channelId)) {
					...
					this.debug('Action on non existing channel: ' + channelId);
					break;
				}
				if (!this._checkValidLayoutId(channelId, layoutId)) {
					...
					this.debug('Action on non existing layout "' + layoutId + '" on channel "' + channelId + '"');
					break;
				}
			...
			case 'channelStreaming': {
				...
				if (!this._checkValidPublisherId(channelId, publishersId)) {
					...
					this.debug('Action on non existing publisher "' + publishersId + '" on channel "' + channelId + '"');
					break;
				}
				...
				if (startStopAction === null) {
					...
					this.debug('The was called for a unknow action: ' + action.options.startStopAction);
					break;
				}
			...
			case 'recorderRecording': {
				...
				if (!this._getRecorderById(recorderId)) {
					...
					this.debug('Action on non existing recorder: ' + recorderId);
					break;
				}
	...
	async _updateRecorderStatus() {
		...
		for (const a in recoders) {
			...
			const currentRecorder = this._getRecorderById(recoder.id);
			if (currentRecorder === undefined) {
				continue;
			}
